        ...
        if (sender.uid !== ownerUid) {
            return (
                <div className='d-flex mb-3' style={{ alignItems: "flex-start" }}>
                    ...
                    <div style={{ marginLeft: "10px" }}>
                        ...
                        <p className='p-0 m-0 mb-1' style={{ color: sender.uid === auth.currentUser.uid?"#000":'grey', fontSize: "10px" }}><i class="fa-solid fa-envelope"></i> {sender.email}</p>
            ...
            return (
                <div className='d-flex mb-3' style={{ alignItems: "flex-start", justifyContent: "end" }}>
                    <div style={{ marginLeft: "10px" }}>
                        <p className='p-0 m-0' style={{ color: sender.uid === auth.currentUser.uid ? '#000' : 'grey', fontSize: "12px" }}>@<b>{sender.name}</b> ~<small><Moment fromNow>{dateAdded}</Moment></small></p>
                        <p className='p-0 m-0 mb-1' style={{ color: sender.uid === auth.currentUser.uid?"#000":'grey', fontSize: "10px", textAlign: "end" }}><i class="fa-solid fa-envelope"></i> {sender.email}</p>
